...
> **ADR と `docs/` の役割関係**
>
> ADR は **判断の歴史** を記録する場所で、Accepted な ADR の本文は書き換えない (撤回時は新規 ADR + `Superseded by NNNN`)。一方、リポジトリルートの [`docs/`](../../docs/) は **現状のスナップショット** (今のシステムがどう実装されているか) を記述する場所で、コードと一緒に更新される。同じトピックでも、「なぜそうしたか」は ADR、「いまどうなっているか」は `docs/` を見る。両者が矛盾するように見える時は、ADR は当時の判断、`docs/` がいまの実装の真実、という解釈になる。
